...
class ConvNet(nn.Module):
    def __init__(self, stride1, kernel_size1, stride2, kernel_size2):
        ...
        self.conv1 = nn.Conv2d(3, 10, kernel_size=kernel_size1, stride=stride1)
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            if misclassified_mask.any():
                misclassified_mask_expanded = misclassified_mask.unsqueeze(2).unsqueeze(3).expand_as(data)
                misclassified_data = data[misclassified_mask_expanded].view(-1, 3, 32, 32)
                misclassified_target = target[misclassified_mask.view(-1)][0]
...
def visualize_misclassified(misclassified_images, outputdir, num=10):
    ...
    for i in range(0, num):
        ...
        img = misclassified_images[i][0][0].cpu().numpy().transpose((1, 2, 0))
        ax.imshow(img)
...
def visualize_correct_predictions(correct_images, num_rows, num_cols):
    ...
    for i, (image, true_label, predicted_label) in enumerate(correct_images[:num_rows * num_cols]):
        plt.subplot(num_rows, num_cols, i + 1)
        plt.imshow(image.permute(1, 2, 0).cpu().numpy(), cmap='viridis')
...
if __name__ == '__main__':
    ...
    outputdir = "./ciphar_result/s" + str(stride1) + "_k" + str(kernel_size1) + "_s" + str(stride2) + "_k" + str(kernel_size2)
